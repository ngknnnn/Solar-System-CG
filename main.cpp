...
public:
	...
	// create constructor to create radius, distance, speed 
...
// calling the variety of planets with neccessary data 
...
GLuint loadTexture(Image* image) { // Make a image into a texture
	...
	glBindTexture(GL_TEXTURE_2D, textureId); // tell the openGL what texture to edit
...
// declaring the name of different planets
...
void drawScene(void){
	...
	gluSphere(quadric, sun.radius, 20.0, 20.0);// quadric object, radius,slices (Specifies the number of subdivisions around the z axis (similar to lines of longitude).) , stacks(latitude)
...
void keyInput(unsigned char key, int x, int y){
	...
	case '8': if (smallOrbitActive) smallOrbitActive = 0; else smallOrbitActive = 1; glutPostRedisplay(); break;
	case '9': if (bigOrbitActive) bigOrbitActive = 0; else bigOrbitActive = 1; glutPostRedisplay(); break;
...
void intructions(void){
	...
	cout << "9 to show/hide Big Orbital Trails." << endl;
	cout << "8 to show/hide Small Orbital Trails." << endl;
